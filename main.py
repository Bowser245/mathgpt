...
# Verification et installation automatique de Ollama selon l'OS
def verifier_et_installer_ollama():
    print("[DEBUG] Verification de la presence de Ollama")
    try:
        # On verifie si la commande 'ollama' est disponible dans le systeme
        subprocess.run(["ollama", "--version"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=True)
        print("[DEBUG] Ollama est deja installe")
    except (subprocess.CalledProcessError, FileNotFoundError):
        systeme = platform.system()
        print(f"[DEBUG] Ollama manquant sur {systeme}. Lancement de l'installation")
        
        try:
            if systeme == "Windows":
                # Installation silencieuse via winget pour Windows
                subprocess.check_call(["winget", "install", "Ollama.Ollama", "--silent", "--accept-source-agreements", "--accept-package-agreements"])
                print("[DEBUG] Installation de Ollama terminee avec succes sur Windows")
            
            elif systeme == "Linux":
                # Installation officielle via le script d'Ollama pour Linux
                print("[DEBUG] Telechargement et execution du script d'installation Linux")
                cmd = "curl -fsSL https://ollama.com/install.sh | sh"
                subprocess.check_call(cmd, shell=True)
                print("[DEBUG] Installation de Ollama terminee avec succes sur Linux")
            
            elif systeme == "Darwin":  # macOS
                # Installation via Homebrew pour macOS s'il est present
                print("[DEBUG] Tentative d'installation via Homebrew sur macOS")
                subprocess.check_call(["brew", "install", "--cask", "ollama"])
                print("[DEBUG] Installation de Ollama terminee avec succes sur macOS")
            
            else:
                print(f"[DEBUG] OS non supporte pour l'installation automatique : {systeme}")
                
        except Exception as e:
            print(f"[DEBUG] Impossible d'installer Ollama automatiquement : {str(e)}")

# Execution de la verification de Ollama avant de lancer l'interface
verifier_et_installer_ollama()

# Verification et installation automatique de PyQt5
try:
    from PyQt5 import QtCore, QtGui, QtWidgets
except ImportError:
    print("[DEBUG] PyQt5 manquant. Installation en cours")
...
class UntardleApp(QtWidgets.QWidget):
    ...
    def process_compression(self):
        ...
        print("[DEBUG] Debut de la compression")
        ...
        try:
            with open(output_path, "w", encoding="utf-8") as f_out:
                ...
                for root, dirs, files in os.walk(self.source_dir):
                    for file in files:
                        ...
                        print(f"[DEBUG] Archivage de : {formatted_key}")
                        ...
                        try:
                            ...
                            print(f"[DEBUG] Erreur de lecture pour {file} : {str(e)}")
                            b64_data = ""
            ...
            print("[DEBUG] Compression terminee avec succes")
            QtWidgets.QMessageBox.information(self, "Succès", f"Archive Untardle créée avec succès :\n{output_path}")
            
        except Exception as e:
            print(f"[DEBUG] Erreur durant l'ecriture : {str(e)}")
    ...
    def process_decompression(self):
        ...
        print("[DEBUG] Debut de la decompression")
        ...
        try:
            ...
            for line in lines[1:]:
                ...
                if "</file-end/>" in line and inside_file:
                    ...
                    try:
                        ...
                        print(f"[DEBUG] Erreur decodage base64 : {str(e)}")
                        decoded_bytes = b""
                        
                    print(f"[DEBUG] Extraction binaire de : {current_file_path}")
            ...
            print("[DEBUG] Decompression terminee avec succes")
            QtWidgets.QMessageBox.information(self, "Succès", "Extraction terminée avec succès !")
            
        except Exception as e:
            print(f"[DEBUG] Erreur durant l'extraction : {str(e)}")
